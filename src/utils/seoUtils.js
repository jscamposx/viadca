...
export const generatePackageKeywords = (paquete) => {
  ...
  const keywords = [
    ...
    ...(paquete.descripcion ? extractActivityKeywords(paquete.descripcion) : [])
  ];
...
const extractActivityKeywords = (text) => {
  ...
  const activityTerms = [
    'excursión', 'excursiones', 'tour', 'tours', 'visita', 'visitas',
    'playa', 'playas', 'montaña', 'montañas', 'ciudad', 'ciudades',
    'museo', 'museos', 'parque', 'parques', 'aventura', 'aventuras',
    'cultura', 'cultural', 'gastronómico', 'gastronomía',
    'naturaleza', 'natural', 'histórico', 'historia',
    'arqueológico', 'arqueología', 'colonial',
    'spa', 'relax', 'wellness', 'descanso',
    'buceo', 'snorkel', 'pesca', 'senderismo', 'trekking',
    'cenotes', 'cascadas', 'volcanes', 'ruinas'
  ];
...
export const generateSEOTitle = (paquete) => {
  ...
  const destinos = paquete.destinos?.map(d => d.ciudad || d.destino).filter(Boolean).slice(0, 2) || [];
...
export const generateSEODescription = (paquete) => {
  if (!paquete) return 'Descubre increíbles paquetes de viaje con Viadca Viajes, tu agencia de confianza desde Durango.';
  ...
  const incluye = paquete.incluye ? `. Incluye: ${paquete.incluye.substring(0, 80)}...` : '';
  ...
  return baseDesc.length > 155 ? 
    baseDesc.substring(0, 152) + '...' : 
    baseDesc;
};
...
export const generatePackageJsonLd = (paquete, url) => {
  ...
  const destinos = paquete.destinos?.map(d => d.ciudad || d.destino).filter(Boolean) || [];
  ...
  return [
    ...
    // Product Schema
    {
      ...
      offers: {
        ...
        availability: paquete.activo ? "https://schema.org/InStock" : "https://schema.org/Discontinued",
        seller: {
          "@type": "Organization",
          name: "Viadca Viajes",
    ...
    // TourPackage Schema
    {
      ...
      itinerary: paquete.destinos?.map((dest) => ({
