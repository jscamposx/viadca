...
const AdminPaquetes = () => {
  ...
  useEffect(() => {
    if (paquetes) {
      ...
      if (searchTerm) {
        ...
        result = result.filter(
          (p) =>
            ...
            (p.destinos && p.destinos.some(d => d.destino?.toLowerCase().includes(term)))
        );
      }
      ...
      if (mayoristaFilter) {
        result = result.filter((p) => {
          return p.mayoristas && p.mayoristas.some(m => m.id === mayoristaFilter);
        });
      }

      // Filtro por tipo de producto (basado en mayoristas)
      if (tipoProductoFilter) {
        result = result.filter((p) => {
          return p.mayoristas && p.mayoristas.some(m => m.tipo_producto === tipoProductoFilter);
        });
      }
      ...
      if (sortConfig.key) {
        result.sort((a, b) => {
          ...
          // Manejo especial para precios
          if (sortConfig.key === "precio_total") {
            aValue = parseFloat(aValue) || 0;
            bValue = parseFloat(bValue) || 0;
          }

          // Manejo especial para booleanos (activo)
  ...
  }, [paquetes, searchTerm, sortConfig, priceFilter, mayoristaFilter, statusFilter, tipoProductoFilter]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-md p-4 sm:p-5 lg:p-6 mb-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            {/* Barra de búsqueda - ocupa más espacio en desktop */}
            ...
            <div className="flex flex-col sm:flex-row gap-2 lg:col-span-3">
              <div className="bg-blue-50 text-blue-700 py-2 px-3 rounded-xl font-medium text-xs sm:text-sm flex items-center gap-2 justify-center flex-1">
                ...
                <span className="hidden sm:inline">{filteredPaquetes.length === 1 ? "paquete" : "paquetes"}</span>
                <span className="sm:hidden">pqts</span>
              </div>

              {/* Estadísticas adicionales - solo en desktop */}
              {paquetes && (
                <div className="hidden lg:flex gap-2 flex-1">
                  ...
                  <div className="bg-purple-50 text-purple-700 py-2 px-3 rounded-xl font-medium text-xs flex items-center gap-1 justify-center flex-1">
                    ...
                    <span className="font-bold">
                      {new Set(paquetes.flatMap(p => p.mayoristas?.map(m => m.id) || [])).size}
                    </span>
                  </div>
                </div>
              )}
              ...
              {(searchTerm || priceFilter.min || priceFilter.max || mayoristaFilter || statusFilter || tipoProductoFilter) && (
                <button
            ...
            <div className="mt-4 p-4 sm:p-5 bg-gray-50 rounded-xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                ...
                <div>
                  ...
                  >
                    <option value="">Todos</option>
                    {mayoristas && Array.from(new Set(mayoristas.map(m => m.tipo_producto)))
                      .filter(Boolean)
                      .map((tipo) => (
                ...
                <div className="sm:col-span-2 lg:col-span-3 flex flex-col sm:flex-row justify-between items-center pt-4 gap-3">
                  <span className="text-sm text-gray-600 order-2 sm:order-1">
                    {filteredPaquetes.length} paquete{filteredPaquetes.length !== 1 ? 's' : ''} encontrado{filteredPaquetes.length !== 1 ? 's' : ''}
                  </span>
        ...
        <div className="bg-white rounded-2xl shadow-md p-3 sm:p-4 mb-6">
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            {/* Mostrar menos filtros en móvil */}
                  ...
                  <div className="p-4 lg:p-6">
                    <div className="mb-4 lg:mb-5">
                      ...
                      <div className="flex flex-wrap gap-2 lg:gap-3 mb-3 lg:mb-4">
                        {/* Estado del paquete */}
                        <span className={`px-3 py-1.5 rounded-full text-sm lg:text-base font-medium ${
                          paquete.activo 
                        ...
                        {paquete.mayoristas && paquete.mayoristas.length > 0 && (
                          <span className="px-3 py-1.5 rounded-full text-sm lg:text-base font-medium bg-purple-100 text-purple-700 flex items-center gap-2">
                            ...
                            <span className="hidden sm:inline">
                              {paquete.mayoristas.length} mayorista{paquete.mayoristas.length > 1 ? 's' : ''}
                            </span>
                            <span className="sm:hidden">
                        ...
                        {paquete.destinos && paquete.destinos.length > 0 && (
                          <span className="px-3 py-1.5 rounded-full text-sm lg:text-base font-medium bg-orange-100 text-orange-700 flex items-center gap-2">
                            ...
                            <span className="hidden sm:inline">
                              {paquete.destinos.length} destino{paquete.destinos.length > 1 ? 's' : ''}
                            </span>
                            <span className="sm:hidden">
                      ...
                      {/* Mayoristas asociados - solo en pantallas grandes */}
                      {paquete.mayoristas && paquete.mayoristas.length > 0 && (
                        <div className="mb-3 lg:mb-4 hidden sm:block">
                          <p className="text-sm lg:text-base text-gray-500 mb-2">Mayoristas:</p>
