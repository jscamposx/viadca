...
const fieldMeta = [
  { key: 'telefono', label: 'Teléfono', icon: FiPhone, type: 'tel', placeholder: '+51 999 999 999', group: 'contacto' },
  { key: 'email', label: 'Email', icon: FiMail, type: 'email', placeholder: 'contacto@viadca.app', group: 'contacto' },
  { key: 'whatsapp', label: 'WhatsApp', icon: FiPhone, type: 'tel', placeholder: '+51 999 999 999', group: 'contacto' },
  { key: 'direccion', label: 'Dirección', icon: FiMapPin, type: 'text', placeholder: 'Av. Principal 123', group: 'contacto' },
  { key: 'horario', label: 'Horario', icon: FiClock, type: 'text', placeholder: 'L-V 9:00-18:00', group: 'contacto' },
  { key: 'facebook', label: 'Facebook', icon: FiFacebook, type: 'url', placeholder: 'https://facebook.com/viadca', group: 'redes' },
  { key: 'instagram', label: 'Instagram', icon: FiInstagram, type: 'url', placeholder: 'https://instagram.com/viadca', group: 'redes' },
  { key: 'tiktok', label: 'TikTok', icon: FiEdit2, type: 'url', placeholder: 'https://tiktok.com/@viadca', group: 'redes' },
  { key: 'youtube', label: 'YouTube', icon: FiYoutube, type: 'url', placeholder: 'https://youtube.com/@viadca', group: 'redes' },
];
...
const AdminConfigPage = () => {
  ...
  const handleClear = async () => {
    if (!window.confirm('¿Estás seguro de que deseas limpiar todos los campos? Esta acción no se puede deshacer.')) return;
    setSaving(true);
    setMessage({ text: '', type: '' });
    try {
      ...
      setInitial({ ...EMPTY, ...normalized });
      setData({ ...EMPTY, ...normalized });
      setMessage({ text: 'Todos los campos fueron limpiados', type: 'success' });
    } catch (e) {
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 sm:items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600">Administra la información de contacto pública</p>
              </div>
            </div>
          ...
          <div className="mt-6 pt-5 border-t border-gray-100 flex justify-between items-center">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${
                loading || saving ? 'bg-blue-500 animate-pulse' : 
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div className="mb-5 pb-3 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-600 mt-1">Detalles principales para que los clientes puedan contactarte</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {groupedFields.contacto.map(({ key, label, icon: Icon, type, placeholder }) => (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div className="mb-5 pb-3 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-600 mt-1">Enlaces a tus perfiles en redes sociales</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {groupedFields.redes.map(({ key, label, icon: Icon, type, placeholder }) => (
                <div key={key} className="space-y-2">
                  <label htmlFor={key} className="block text-sm font-medium text-gray-700">
                    {label}
                  </label>
        ...
        <div className="mt-6 bg-blue-50 border border-blue-100 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-blue-700">
              Todos los campos son opcionales. Los cambios se aplicarán inmediatamente después de guardar.
              Las URLs deben incluir el protocolo (ej: https://).
            </p>
          </div>
        </div>
      </div>
