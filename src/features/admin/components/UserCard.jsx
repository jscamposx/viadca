...
const UserCard = ({
  ...
  const displayUsername = isUsernameTruncated
    ? rawUsername.slice(0, MAX_NAME_LEN) + "…"
    : rawUsername;
      ...
      <div className="group bg-white rounded-2xl sm:rounded-3xl shadow-lg overflow-visible border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col h-full">
        ...
        <div className="relative p-4 sm:p-5 lg:p-6 pr-14 sm:pr-16 lg:pr-20 bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50">
          ...
          <div className="flex items-center gap-4">
            {/* Avatar / Ícono */}
            <div
              className={`${getRoleIconBg(user.rol)} p-3 sm:p-4 rounded-xl shadow-md transition-all duration-300 flex items-center justify-center`}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              <h3 className="text-lg sm:text-xl font-bold text-gray-900 group-hover:text-purple-600 transition-colors duration-200">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="relative flex-1 min-w-0" title={rawUsername}>
                    <span className="block truncate pr-2 sm:pr-4">
                      {displayUsername}
                    </span>
                    {isUsernameTruncated && (
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span
                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-medium bg-white/80 shadow-sm ${
                    user.rol === "admin"
                      ? "text-red-700"
                ...
                <span
                  className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-medium bg-white/80 shadow-sm ${isEmailVerified(user) ? "text-green-700" : "text-yellow-700"}`}
                >
          ...
          {roleMenuOpen && (
            <div
              ref={menuRef}
              className="absolute z-50 mt-2 right-3 top-12 w-60"
            >
              <div className="p-0.5 rounded-2xl bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-600 shadow-xl">
                <div className="rounded-2xl bg-white/90 backdrop-blur-xl border border-white/50 overflow-hidden">
                  <div className="px-3 py-2 border-b border-indigo-100/60 flex items-center justify-between">
                    <span className="text-[11px] font-semibold tracking-wide text-indigo-600">
                      Cambiar rol
                    </span>
                    <button
                      onClick={() => setRoleMenuOpen(false)}
                      className="p-1 rounded-md hover:bg-indigo-50 text-indigo-500"
                    >
                    ...
                    */}
                    {["usuario", "pre-autorizado", "admin"].map((role) => (
                      <li key={role}>
                        ...
                        >
                          <span
                            className={`mt-0.5 ${user.rol === role ? "text-indigo-600" : "text-indigo-500 group-hover:scale-110 transition-transform"}`}
                          >
                            {role === "admin" ? (
                          ...
                          <span className="flex-1 min-w-0">
                            ...
                            <span
                              className={`text-[10px] leading-tight opacity-80 ${user.rol === role ? "text-indigo-600" : "text-gray-500"}`}
                            >
                              {role === "admin"
                                ? "Control total"
                                : role === "pre-autorizado"
        ...
        <div className="p-4 sm:p-5 lg:p-6 flex-1 flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            <div className="bg-blue-50 hover:bg-blue-100 rounded-lg p-3 text-center transition-all duration-200 hover:shadow-md hover:scale-105 cursor-default">
              ...
              <div
                className="text-[11px] text-blue-700 font-medium truncate"
                title={getRegistrationDate(user)}
              >
                {getRegistrationDate(user)}
              </div>
            </div>
            <div
              className={`rounded-lg p-3 text-center transition-all duration-200 hover:shadow-md hover:scale-105 cursor-default ${isEmailVerified(user) ? "bg-green-50 hover:bg-green-100" : "bg-yellow-50 hover:bg-yellow-100"}`}
            >
              {isEmailVerified(user) ? (
                <FiCheckCircle className="w-4 h-4 mx-auto mb-1 text-green-600" />
              ) : (
                <FiClock className="w-4 h-4 mx-auto mb-1 text-yellow-600" />
              )}
              <div
                className={`text-[11px] font-medium truncate ${isEmailVerified(user) ? "text-green-700" : "text-yellow-700"}`}
              >
            ...
            <div className="mb-5">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span
                    className="text-sm font-medium text-gray-900 truncate max-w-[55%]"
                    title={user.correo || user.email}
                  >
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="text-sm font-medium text-gray-900 truncate max-w-[55%]">
                    {getRoleLabel(user.rol)}
                  </span>
                </div>
              </div>
