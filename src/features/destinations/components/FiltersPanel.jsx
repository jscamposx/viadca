    ...
    >
      ...
      >
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <input
                ...
                className="px-3 py-2 rounded-lg border border-slate-200 bg-white focus:ring-2 focus:ring-blue-600/50 outline-none text-sm"
              />
              <input
                type="number"
            ...
            <div className="flex flex-wrap gap-2">
              {TIPOS.map((t) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium border transition ${values.tipos.includes(t) ? "bg-blue-600 text-white border-blue-600 shadow" : "bg-white text-slate-600 border-slate-300 hover:border-slate-400"}`}
                >
                  {t}
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-2">
              {CONTINENTES.map((c) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium border transition ${values.continentes.includes(c) ? "bg-indigo-600 text-white border-indigo-600 shadow" : "bg-white text-slate-600 border-slate-300 hover:border-slate-400"}`}
                >
                  {c}
                </button>
              ))}
        ...
        <div className="p-5 border-t border-slate-100 flex items-center justify-between gap-3">
          <button
            onClick={reset}
            className="inline-flex items-center gap-2 px-4 py-2 text-slate-500 hover:text-slate-700 text-sm font-medium"
          >
            <FiRotateCcw className="w-4 h-4" /> Reset
          </button>
          <button
            onClick={apply}
            className="px-6 py-2.5 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-sm font-semibold shadow hover:from-blue-700 hover:to-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-600"
          >
            Aplicar filtros
          </button>
        </div>
