...
const RouteMap = ({ paquete }) => {
  ...
  const { originPosition, destinationPosition, polylinePositions, bounds } = useMemo(() => {
    const origin = [
    ...
    const isValid = (lat, lng) => !isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180;

    if (!isValid(origin[0], origin[1]) || !isValid(destination[0], destination[1])) {
        return { valid: false };
    }
      ...
      <div className=" bg-gray-50 rounded-lg ">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-center">
          {paquete.tracking_number && (
            <div>
              <div className="font-semibold text-gray-700">Nº de Seguimiento</div>
