  ...
  >
    <div className="max-w-7xl mx-auto">
      <div className="grid lg:grid-cols-2 gap-10 sm:gap-12 lg:gap-16 items-start">
        ...
        <div className="space-y-6 lg:space-y-8 order-1 relative">
          ...
          <div className="space-y-5 sm:space-y-6 lg:space-y-8">
            {steps.map((s,i)=>(
              ...
              >
                <div className={`w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-gradient-to-r ${s.color} rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg group-hover:scale-110 transition-transform duration-300 motion-safe:animate-none`} aria-hidden="true">
                  <svg className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={s.iconPath} /></svg>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
                    <h3 className="font-bold text-base sm:text-lg lg:text-xl text-slate-800">{s.title}</h3>
                    <span className={`${s.bgBadge} px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-medium`}>{s.badge}</span>
                  </div>
        ...
        {/* Right card */}
        <AnimatedSection animation="fadeInRight" delay={400} className="relative order-2">
          <div className="relative bg-white rounded-3xl p-4 sm:p-6 shadow-xl border border-blue-100 w-full sm:max-w-lg lg:max-w-md mx-auto hover:shadow-2xl transition-shadow duration-300">
            ...
            <div className="flex items-center justify-center sm:justify-start gap-4 mb-4 sm:mb-6">
              {['Transporte','Comidas','Guía'].map(label => (
                <div key={label} className="flex flex-col items-center">
                  ...
                  <span className="text-[10px] sm:text-xs text-slate-600 mt-1">{label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="text-left sm:text-right">
                <div className="text-[11px] sm:text-xs text-slate-500">Desde</div>
          ...
          <div className="lg:hidden mt-5 grid grid-cols-3 gap-3">
            {[['24/7','Soporte','blue'],['15+','Años','green'],['100%','Seguro','orange']].map(([v,l,c])=> (
              <div key={l} className="bg-white rounded-xl p-2.5 shadow-sm border border-blue-100 text-center">
